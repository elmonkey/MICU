...
class ARCtable(tb.IsDescription):
    ...
    timestamp       = tb.Float32Col(shape=(),           pos=2) # system time stamp for the frame
    viewangle       = tb.Int32Col  (shape=(),   dflt=0, pos=3) # view angle
    actionlabel     = tb.Int32Col  (shape=(),   dflt=0, pos=4) # action int label
    actorlabel      = tb.Int32Col  (shape=(),   dflt=0, pos=5) # action int label
    pose            = tb.StringCol (itemsize=(20), shape=(),   dflt=0, pos=6)  # action string name
    actorname       = tb.StringCol  (itemsize=(20), shape=(),   dflt=0, pos=7) # actor string name
    timestring      = tb.StringCol  (itemsize=(40), shape=(),   dflt=0, pos=8) # time as str
